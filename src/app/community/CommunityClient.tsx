 ...
 * @description 커뮤니티 페이지 클라이언트 컴포넌트.
 * "use client" 컴포넌트에서 서버 액션을 직접 임포트하는 구조는 RSC 빌드 오류 유발 가능.
 * page.tsx(RSC 래퍼)가 이 컴포넌트를 렌더링하도록 분리.
 */
"use client";
...
export default function CommunityClient() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getPostsByCategory(activeCategory as Parameters<typeof getPostsByCategory>[0]).then((data) => {
      setPosts(data);
      setLoading(false);
    });
  }, [activeCategory]);
